...
export class ChessGame {
  ...
  private updateBoardDisplay(): void {
    ...
    if (this.selectedPosition) {
      const selectedCell = this.getCellElement(
  ...
  private handleSquareClick(event: Event): void {
    if (this.gameResult !== "ongoing") return;
  ...
  private makeMove(from: Position, to: Position): boolean {
    ...
    const kingInCheck = PieceLogic.isKingInCheck(
      this.board,
      this.currentPlayer
    );
  ...
  private makeBotMove(): void {
    if (this.currentPlayer !== "black" || this.gameResult !== "ongoing") return;
  ...
  private updateUI(): void {
    ...
    if (currentPlayerElement) {
      const playerText =
        this.currentPlayer === "white" ? "Хід білих" : "Хід чорних";
