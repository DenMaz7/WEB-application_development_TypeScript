// history.ts - збереження та читання з games.json
...
export class GameHistory {
  private readonly STORAGE_KEY = "chess_game_history";
  ...
  public async saveGame(gameData: {
    startTime: Date;
    endTime: Date;
    gameMode: "local" | "bot";
    moveCount: number;
    ...
    const duration = Math.floor(
      (gameData.endTime.getTime() - gameData.startTime.getTime()) / 1000
    );
  ...
  public async displayHistory(): Promise<void> {
    ...
    let historyHTML =
      '<h3 style="margin-bottom: 20px; color: #4a5568;">📜 Останні партії</h3>';
    ...
    this.games.forEach((game, index) => {
      ...
      const modeStr =
        game.gameMode === "local" ? "👥 Локальна гра" : "🤖 Гра проти бота";
  ...
  private loadGames(): void {
    try {
      ...
      if (storedGames) {
        ...
        this.games = parsedGames.map((game: any) => ({
          ...game,
