// bot.ts - логіка бота
import { ChessPiece, PieceColor, Position, PieceLogic } from "./pieces.js";
...
export class ChessBot {
  ...
  public getBestMove(
    ...
    const goodMoves = evaluatedMoves.filter(
      (move) => move.score >= bestScore - 0.5
    );
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        if (piece && piece.color === color) {
          ...
          const possibleMoves = PieceLogic.getPossibleMoves(
            board,
            piecePosition
          );
  ...
  private evaluatePawnPosition(position: Position, color: PieceColor): number {
    let score = 0;
  ...
  private evaluateKnightPosition(position: Position): number {
    // Коні краще в центрі
    const distanceFromCenter =
      Math.abs(3.5 - position.row) + Math.abs(3.5 - position.col);
    return (7 - distanceFromCenter) * 0.1;
  }

  private evaluateBishopPosition(position: Position): number {
  ...
  private evaluateRookPosition(
    ...
    if (openFile) score += 0.3;
    if (openRank) score += 0.3;

    return score;
  }

  private evaluateQueenPosition(position: Position): number {
  ...
  private evaluateKingPosition(position: Position, color: PieceColor): number {
    let score = 0;
