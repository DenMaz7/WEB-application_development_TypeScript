...
Буде видалено ${this.gameHistory.length} записів.`;
  ...
  private renderGameHistory(): void {
  ...
  private placeShip(row: number, col: number): void {
    ...
    const currentShip = shipsList[this.placingShipIndex];
    const currentBoard =
      this.placingPlayer === "player1" ? this.player1Board : this.player2Board;
    ...
    if (this.placingShipIndex < shipsList.length - 1) {
      ...
      this.message = `${
        this.placingPlayer === "player1" ? "Гравець 1" : "Гравець 2"
      }: розмістіть корабель розміром ${nextShip.size}`;
    } else {
  ...
  private placeBotsShips(): void {
    ...
    shipsList.forEach((ship) => {
      ...
      while (!placed && attempts < 100) {
        ...
        if (
          this.canPlaceShip(this.player2Board, row, col, ship.size, horizontal)
        ) {
          const positions: Position[] = [];
  ...
  private async makeShot(row: number, col: number, targetPlayer: Player): Promise<void> {
    ...
    const targetBoard =
      targetPlayer === "player1" ? this.player1Board : this.player2Board;
    const targetShips =
      targetPlayer === "player1" ? this.player1Ships : this.player2Ships;
  ...
  private botMove(): void {
    ...
    if (this.botTargetQueue.length > 0) {
      ...
      do {
        ...
        (this.player1Board[row][col].state === "hit" ||
